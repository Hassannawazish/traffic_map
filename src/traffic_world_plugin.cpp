...
class TrafficWorldPlugin : public WorldPlugin {
 ...
 public:void Load(physics::WorldPtr world,sdf::ElementPtr) override {world_=world; const char *path=std::getenv("TRAFFIC_ROUTE_CSV"); if(!path)return;
    ...
    for(std::size_t i=0;i<count;++i){
      ...
      distance_[i]=i==0?40.:55.+(i-1)*12.;speed_[i]=i==0?20.:6.+.7*i;direction_[i]=1.;lateral_[i]=3.2*(lane+1);
      ...
      if(i==0){
        sdf<<"<visual name='dashboard_sensor_housing'><pose>1.15 0 .72 0 0 0</pose><geometry><box><size>.16 .32 .12</size></box></geometry>"
          ...
          <<"<range><min>.20</min><max>250</max><resolution>.02</resolution></range><noise><type>gaussian</type><mean>0</mean><stddev>.01</stddev></noise></ray>"
  ...
  void OnUpdate(){auto now=world_->SimTime();double dt=(now-last_).Double();if(dt<1.0/120.0)return;last_=now;for(std::size_t i=0;i<distance_.size();++i){auto model=world_->ModelByName("traffic_vehicle_"+std::to_string(i));if(!model)continue;
